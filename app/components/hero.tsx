...
function FanClubControl({ isMounted }: { isMounted: boolean }) {
  ...
  if (!hasAnyUserSession) {
    return (
      ...
      >
        <SignUpButton mode="modal" appearance={spaceClerkAppearance}>
          <MoonButtonVisual />
        </SignUpButton>
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden">
      ...
      {hasClerkPublishableKey ? (
        <FanClubControl isMounted={isMounted} />
      ) : (
        isMounted && (
          <motion.div
            className="absolute top-6 right-6 z-20"
